	...
	<view class="content">
			...
			<scroll-view :scroll-top="scrollTop" scroll-y="true" class="scroll-Y" style="height: 640px;">
				<view class="bg-white module-one">
					...
					<view class="module-one-font">
						<view>特色科室: {{hospitalInfo.hosName}}</view>
						<view>地址: {{hospitalInfo.address}}</view>
					</view>
				</view>

				<view class="content-list">
					<view class="bg-white list-item mt-20" v-for="(item,index) in hospitalInfoList" :key="index">
						<view class="list-contents">
							<view class="list-top">
								<view class="list-title">联系部门</view>
								<view class="list-span">{{item.det}}</view>
							</view>
		...
		<view class="modal" v-if="showModal">
			<view class="modal-content">
				...
				<view v-for="(item,index) in hospitalList" @click="selectH(item)" style="text-align: center;padding-top: 20px;">
					{{ item.hosName }}
				</view>
			</view>
		</view>
	...
	export default {
		...
		mounted() {
			...
			getHospitalList().then(res => {
				this.navTitle = res.data[0].hosName
				this.navTitleId = res.data[0].id
				this.hospitalList = res.data
				uni.setStorageSync('hospitalId',this.navTitleId);
			}).then(()=>{
				this.HospitalInfo(this.navTitleId)
			})
		...
		methods: {
			selectH(val) {
				this.navTitle = val.hosName
				this.HospitalInfo(val.id)
				this.showModal=false
				uni.setStorageSync('hospitalId',val.id);
			},
			HospitalInfo(val) {
				getHospitalInfo(val).then(res => {
					this.hospitalInfo = res.data
					this.hospitalInfoList = res.data.list
				})
			}
		}
	}
